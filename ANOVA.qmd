...
```{r}
library(rstatix)
df |> group_by(method,Days) |> 
    get_summary_stats(type="mean_sd")

# model <-df |> rstatix::anova_test(dv=volume,wid=id,within = Days,between = method)
...
```{r}
# 医学统计学 人卫版 第7版 Page 86-87
df <- read_tsv("data/麻醉诱导时相.txt")
df
```
...
&=（SS_{处理方法}+SS_{个体差异}）+（SS_{时间}+SS_{处理与时间交互}+SS_{个体差异}）
\end{aligned}
$$
...
```{r}
id_mean <- df |> select(c(-1,-2)) |> rowMeans()
...
df_long <- df |> pivot_longer(cols = starts_with("t"),
                              names_to = "time",
                              values_to = "BP")
Grouped_mean <- df_long |> group_by(诱导方法) |> 
...
```{r}
SS_处理 <- 25*( (Grouped_mean$mean[1] - BP_mean )^2 + 
                       (Grouped_mean$mean[2] - BP_mean )^2 +
                       (Grouped_mean$mean[3] - BP_mean )^2) 
SS_处理
```
...
![](images/clipboard-1173484901.png)
