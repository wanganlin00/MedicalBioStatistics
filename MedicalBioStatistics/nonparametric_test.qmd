...
非参数检验是不依赖总体分布类型、不估计总体参数的一类假设检验方法。适用于不满足正态性假设的资料或有序分类资料（等级资料）
...
如果样本数据不满足正态性检验就要采用单样本wilcoxon符号秩检验进行计算。使用该检验需要满足的条件是样本值均匀地分布在均值两侧。
...
## 两样本配对 Wilcoxon's signed-rank test
...
## 两样本独立 Wilcoxon's Rank-Sum 检验 (Mann-Whitney U 检验)

当两个连续样本不满足正态分布时，使用Wilcoxon秩和检验进行非参数检验

有序等级资料1. 整理等级频数表2. 确定秩次与平均秩次3. 计算两组秩和4. 计算U值5. 计算Z值与P值
...
## 多样本独立 Kruskal-Wallis rank sum检验
