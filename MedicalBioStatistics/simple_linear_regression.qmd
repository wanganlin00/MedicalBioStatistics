...
简单线性回归用于解释一个数值变量（因变量）和一个自变量之间的线性关系。模型形式一般为：
...
```{r}
lm_tv <- lm(sales ~ TV, data = advertising)
# 模型摘要
summary(lm_tv)

parameters::model_parameters(lm_tv)
performance::performance(lm_tv)
...
predict(lm_tv, new_data = advertising, type = "terms") |>  
    head(n = 10)
...
```         
predict(lm_tv, new_data = advertising, interval = "confidence") |>  
    head(n = 10)
predict(lm_tv, new_data = advertising, interval = "prediction") |>  
    head(n = 10)
```
...
augment(lm_tv, new_data = advertising) |> 
    ...
    geom_linerange(aes(ymin = sales, ymax = .fitted)) +
...
2.  残差的正态性 $\epsilon\sim N(0,\sigma^2)$
...
# 残差应该随机分布在0附近
tibble(
    `Fitted values`=fitted(lm_tv),
    Residuals = residuals(lm_tv)
...
```{r}
tibble(
       StandardizedResiduals =rstandard(lm_tv) ) |>  
...
# 绘制 Scale-Location 图
tibble(
    fitted_values=fitted(lm_tv),
    StandardizedResiduals = rstudent(lm_tv) ,
) |> 
...
```{r}
threshold <- 4 / (nrow(advertising)-length(lm_tv$coefficients)-2)
                  
                 
tibble(
    x = 1:nrow(advertising),
    cooks_distance = cooks.distance(lm_tv),
