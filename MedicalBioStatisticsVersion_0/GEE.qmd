...
$$
D'_i=(\frac{\partial \mu_i}{\partial \beta} )'
$$

其中U(β) 是一个包含待估计参数的函数，D是quasi-deviance，V是方差-协方差矩阵，R（α）是相关性结构矩阵，y 是观测数据，μ 是模型的均值预测值。采用迭代重复加权最小二乘法（iteratively reweighted least squares (IWLS)），偏微分方程估计参数。
