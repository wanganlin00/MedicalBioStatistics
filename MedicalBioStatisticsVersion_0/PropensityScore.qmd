...
```{r}
dir <- colMeans(x) - colMeans(y)
trt <- x
ctrl <- y
dir_sign <- sign(dir)
# 根据dir的符号分配控制组和处理组
for (i in seq_along(dir_sign)) {
    if (dir_sign[i] > 0) {
        trt[, i] <- x[, i]  # x的这一列作为处理组
        ctrl[, i] <- y[, i]  # y的这一列作为控制组
    } else{
        trt[, i] <- y[, i]  # y的这一列作为处理组
        ctrl[, i] <- x[, i]  # x的这一列作为控制组
    }
}
...
#map_vec(test_results,`$`,estimate)
tau.up <- tau.l <- map_vec(test_results, `$`, estimate)
```

共同支持域的查验

```{r}
sum(glm_ps$fitted.values[treat==1]> max(glm_ps$fitted.values[treat==0]))

sum(glm_ps$fitted.values[treat==1]< min(glm_ps$fitted.values[treat==0]))
```
