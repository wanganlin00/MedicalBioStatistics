...
income1<-read_csv("data/ISLR/Income1.csv")
income2<-read_csv("data/ISLR/Income2.csv")
```
...
# 整理回归模型结果
tidy_lm <- tidy(lm_sales_tv, conf.int=T) %>% dplyr::filter(term !='(Intercept)' )
# 绘制点须图
...
```{r}
autoplot(lm_sales_tv, which = 1:6, ncol = 2, label.size = 3)
```
